...
class PongGameAsync:
    def __init__(self):
        ...
        self.paddle1_position = np.array([0.0, 0.0])  # 패들1의 초기 위치
    ...
    def update_paddle(self, paddle, direction):
        # 방향 벡터 정규화 및 최고속도에 곱하여 속도 계산
        direction = np.array(direction)  # direction 예시: [0, 1] (위쪽), [1, 0] (오른쪽)
        if np.linalg.norm(direction) > 0:  # 방향 벡터가 0이 아닐 때만 정규화
            normalized_direction = direction / np.linalg.norm(direction)
            velocity = normalized_direction * self.paddle_max_speed
        else:
